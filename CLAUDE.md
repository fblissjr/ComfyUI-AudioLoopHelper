...
- `AudioLoopHelperSageAttention` (in `nodes_sage.py`) -- sage attention patch with pytorch fallback, `ON_CLEANUP` handler, and opt-in JSONL telemetry (env: `AUDIOLOOPHELPER_SAGE_TRACE`). Arch-filtered mode combo (no Blackwell footguns on Ada). **Default mode `auto_mask_aware`** routes masked cross-attn calls to `sageattn_qk_int8_pv_fp16_triton` (the one kernel that handles LTX cross-attn cleanly — all CUDA mask kernels produce rtol 0.26–0.94 vs SDPA per `<sage_fork_repo>/tests/test_sageattn_ltx_shapes.py`) and unmasked self-attn to sage `auto` (fp8++ on Ada). Stateless per-call dispatch — no offload/loop state risk. Tracer logs both `mode` (configured) and `effective_mode` (per-call actual kernel) so routing can be confirmed post-hoc. Swap KJ's `PathchSageAttentionKJ` in/out via `scripts/apply_audioloophelper_sage.py [--all] [--revert]`; change the mode widget via `scripts/apply_sage_mode.py <mode|mask_aware>` (handles both node types). User-facing reference at `docs/reference/sage_attention.md`; patch-chain analysis at `internal/analysis/sage_attention_analysis.md`; deferred work at `internal/design/sage_backlog.md`. Tests in `tests/test_sage_node.py`.
