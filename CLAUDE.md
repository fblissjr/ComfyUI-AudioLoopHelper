...
- **LTX2_NAG widgets `[nag_scale, nag_alpha, nag_tau, inplace]`** per source at `ComfyUI-KJNodes/nodes/ltxv_nodes.py:452-459`. There is NO `skip_blocks` parameter — earlier CLAUDE.md annotations incorrectly labelled widget 0 as `skip_blocks`. Workflow default `[11, 0.25, 2.5, True]` = `nag_scale=11` (very aggressive). At scale=11 the attention extrapolation `positive × 11 − negative × 10` can blow out of distribution and freeze/zero the sampler step; dial to 3-7 if you see frozen initial render or stalling. Use `scripts/trace_node_source.py <workflow> 508` to print authoritative schema before editing.
...
- `docs/sampler_reference.md` -- `euler` vs `euler_ancestral` vs `euler_ancestral_cfg_pp` with ComfyUI + MultimodalGuider source code walkthrough; why `euler` is mandatory for our loop architecture and why upstream's `euler_ancestral_cfg_pp` is wrong for merged distilled-1.1
