...
- **fps 25→24 sweep reverted; canonical = 25** (matches Lightricks's shipped workflows + 8n+1 latent boundary). Full evidence + postmortem: `internal/analysis/fps_24_partial_reading_postmortem.md`. Followups: rename `apply_fps_24_default.py`; retire/invert F16+F18; update `frame_rate == 24` test assertions.
