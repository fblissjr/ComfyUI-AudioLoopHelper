...
- **Keyframe schedule**: `KeyframeLatentScheduleBatchEncode` + `LatentSelectByIteration` (timestamp-driven; current — VAE-encodes once outside loop) / `KeyframeImageSchedule` + `ImageBlend` (legacy; per-iter VAE) / `LTXIterKeyframeSchedule` (iter-index-driven, hard frame-N lock via `noise_mask=0` — runs INSIDE loop body for per-iter re-anchoring; DynamicCombo 1-20 image rows, each with `target_iters` string + `target_idx`; staged via `scripts/apply_fml2v_iter_keyframe.py`)
