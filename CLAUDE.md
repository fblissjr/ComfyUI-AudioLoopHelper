...
- `AudioLoopHelperSageAttention` (in `nodes_sage.py`) -- sage attention patch with pytorch fallback, `ON_CLEANUP` handler, and opt-in JSONL telemetry (env: `AUDIOLOOPHELPER_SAGE_TRACE`). Arch-filtered mode combo (no Blackwell footguns on Ada). Drop-in alternative to KJNodes' `PathchSageAttentionKJ` (node 268 in shipped workflows) with the three properties KJ lacks: fallback on exception, cleanup on unload, observability. User-facing reference at `docs/reference/sage_attention.md`; patch-chain analysis at `internal/analysis/sage_attention_analysis.md`; deferred mask-aware routing + accuracy-baselining work at `internal/design/sage_backlog.md`. Tests in `tests/test_sage_node.py`.
