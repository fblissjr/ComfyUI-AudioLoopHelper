...
- **Loop spine**: `AudioLoopController`, `LoopIterationStamp`, `IterationCleanup`, `AudioLoopPlanner`
...
- **Loop-body CONDITIONING must carry `frame_rate=25.0`.** `LTXVConditioning.frame_rate` scales the temporal pos embed at `comfy/ldm/lightricks/av_model.py:866`; batch encoder stamps it, new CONDITIONING-producing loop-body nodes must too via `node_helpers.conditioning_set_values`. Missing → identity drift iter-over-iter. **Canonical LTX 2.3 inference fps = 25** (Lightricks's `ltx-trainer` configs — all modes incl. audio — uniformly set `frame_rate: 25.0`; 8n+1 latent boundary aligns at 25. Official 2.3-series ComfyUI-LTXVideo example workflows say 24 — already investigated, don't re-flip: `internal/analysis/fps_24_partial_reading_postmortem.md` (private clone only)). `LIST_WIDGET_NODES` allowlist coverage enforced by `tests/test_node_schemas.py::test_apply_fps_default_covers_all_fps_bearing_widgets`.
...
- **F14 + F15 output handling**: `VHS_VideoCombine.images` must be fed by `TrimImageBatchToAudio` (eliminates silence-at-end from iter overshoot since ffmpeg `-shortest` doesn't truncate `-c:v copy`); `.filename_prefix` must be fed by `RunIdPrefix` (per-render folder clustering, WARN-level). Latent banking for the LoadLatent upscale/recovery chain lives on `PreDecodeCleanup.checkpoint_keep` (shipped =2; rotated keep-newest-N at `latents/checkpoints/<workflow>_NNNNN_.latent`; replaced standalone `SaveLatent` — `apply_latent_checkpoint.py`, audit `latent_checkpoint`). Guide: `docs/guides/upscale_guide.md`.
...
- **`first_frame_guide_strength` (`FloatConstant #1269`) is the per-iter init-anchor strength** for `LTXVAddLatentGuide #1519`. Ships `0.7` (A/B-validated); `1.0` = max identity lock, minimal motion. Lower = more expressivity, more cross-iter drift: `0.5` soft, `0.3` visible drift, `0.0` no anchor.
...
Internal (gitignored): `internal/PLAN.md`, `internal/ic_lora_assessment.md`, `internal/design/*.md` (long-term designs), `internal/autoresearch/`, `internal/scripts/` (out-of-repo deploy sources), `internal/postmortem_*.md`, `internal/prompts/`, `internal/analysis/`, `internal/log/log_YYYY-MM-DD.md` (session logs).
