...
- **comfy-aimdo's `0 patches attached` log line is misleading.** It counts the legacy `.patches` dict only; modern-API patches (LoRA via `.object_patches`, sage/NAG/AttentionTuner via `transformer_options`) survive offload but don't show in that counter. Confirmed 2026-05-15 via `IterPatchInspector` per-iter inspection: `object_patches=144` constant across iters while comfy-aimdo logs `0 patches attached`. Verify in-flight via `IterPatchInspector` (`nodes.py`), drop it in the model chain inside the loop body.
