...
- **torch profiler op-dispatch and kernel rows are nested, not parallel.** `key_averages()` Self CUDA times for `namespace::op_name` and `void kernel<...>` nest (op covers kernel work + auxiliaries). Summing double-counts. Anchor on kernel rows; treat op rows as call-site metadata. Mismatched call counts (e.g. 4608 vs 3456) are the tell — one op can't fire <1 kernel/call on average.
...
- **fps 25→24 sweep reverted; canonical inference fps = 25.** `apply_fps_24_default.py` (commits `8d9a531`, `b601d85`) flipped 18 workflows on partial reading of `PipelineParams.frame_rate=24.0` (a library placeholder). Lightricks's shipped ComfyUI-LTXVideo examples set `LTXVConditioning.frame_rate=25` across T2V/I2V; V2V uses 24 (preserves source fps). Symptom of 24: initial render fine, loop iters collapsed to near-still (overlap mask + `LTXVAddLatentGuide latent_idx=-1` amplified ~4% pos-embed time-stretch). `--revert` ran (17/19). **Render validation pending.** Followups: rename `apply_fps_24_default.py`; retire/invert F16+F18; update `frame_rate == 24` assertions in `tests/test_node_schemas.py`.
...
- **Top-level model patches may not survive comfy-aimdo's per-iter dynamic-VRAM stage cycle.** Symptom: NAG-disabled at 19.88s window produces frozen loop iters identical to short-window-without-NAG; suggests top-level `LTX2_NAG`-applied patches degrade across iter boundaries even though the count counter shows stable. ComfyUI logs `0 patches attached` (legacy counter, misleading) and `Model LTXAV prepared for dynamic VRAM loading. 24038MB Staged` on EVERY iter — each stage cycle is an opportunity for `object_patches` (NAG/ChunkFFN/AttentionTunerPatch via `model_clone.add_object_patch`) and `model_options["transformer_options"]` (sage via `optimized_attention_override`) to silently fail to re-bind on the GPU module. Source-reading suggests the APIs are designed to survive offload; empirical verification per-workflow is missing. **Test variant**: `example_workflows/experimental/audio-loop-music-video_latent_nag_in_loop.json` (top-level NAG bypassed, NAG node `#2030` inside loop subgraph fed by new subgraph input slot 20 from `CLIPTextEncode #507`). If that variant produces noticeably more loop-body motion than canonical → permanent fix is moving NAG (and possibly sage / ChunkFFN / AttentionTuner) inside the subgraph for per-iter re-apply. **Render validation pending.**
