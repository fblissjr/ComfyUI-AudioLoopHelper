...
- **Top-level model patches may not survive comfy-aimdo's per-iter dynamic-VRAM stage.** Empirical: NAG-disabled at 19.88s = frozen iters (= short-window-no-NAG behavior) hints that `LTX2_NAG`'s `add_object_patch` and sage's `transformer_options["optimized_attention_override"]` degrade across `Model LTXAV prepared for dynamic VRAM loading` cycles even though the source APIs are designed to survive. Test variant: `example_workflows/experimental/audio-loop-music-video_latent_nag_in_loop.json` (NAG inside subgraph, re-executes per-iter via new subgraph slot 20). If motion improves, fix extends to sage / ChunkFFN / AttentionTuner. Render validation pending.
