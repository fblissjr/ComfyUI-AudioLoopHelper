...
- **Testing `ComfyExtension.get_node_list()` requires AST, not invocation.** `AudioLoopHelperExtension.get_node_list()` uses `from .X import Y` relative imports that fail when `nodes.py` is loaded as a top-level module (pytest default — no package context). Use the shared helper `from _node_registry import assert_node_registered` (at `tests/_node_registry.py`) — one call: `assert_node_registered("MyNode")`. Promoted to a helper at the 3rd call site per root CLAUDE.md; now used by 4 node tests (trim_image_batch, latent_frame_count, run_id_prefix, trim_video_latent).
...
- **When audio is short in the saved mp4, walk the audio path FORWARD from `LoadAudio` first.** TrimAudioDuration nodes between `LoadAudio` and the SetNode that exposes `orig_audio` can silently drop content via widget defaults (the 2026-05-10 `#567 [start_index=5, duration=300]` bug ate the first 5s of every song for as long as the canonical existed). Symptom of "audio short by exactly 5s, 10s, etc" is far more often a widget value than a trim-math bug. Order of investigation: source mp3 → `LoadAudio` → `TrimAudioDuration` widgets → `SetNode "orig_audio"` → `VHS_VideoCombine.audio`. F14 latent/image trim only operates on the END of the song; cuts at the START are upstream.
