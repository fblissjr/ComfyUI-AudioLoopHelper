...
### Audio→video IC-LoRA — *training* experiment (process, not a result)

- **Index:** [`../audio_iclora/index.md`](../audio_iclora/index.md) — the self-contained folder for the attempt to *train* an audio-conditioned IC-LoRA (distinct from the inference-only audio-reactive workflows above). Method notes, two-repo map, status.
- **Goal:** a reproducible *process* (data → 4090-fittable trainer → eval) others can fork — not to ship a working LoRA. Trainer side lives in the LTX-2 fork's `docs/`.
- **Status:** pipeline runs end-to-end; the trained LoRA's audio→video behavior is **not yet cleanly measured** (renders so far are confounded by the base's native reactivity). Candid about two dataset leaks and an eval we got wrong twice; recommends a turn-left/turn-right task next. Fork and change whatever you want.
