...
> **Prior experiment, superseded.** This documents the earlier audio→VIDEO coupling direction
> (beat→pulse, turn-left / turn-right). The current effort is the audio→audio pitch IC-LoRA;
> see [`../index.md`](../index.md). Cross-links below are relative to this file's original
> location one directory up, so some may now point a level off.
