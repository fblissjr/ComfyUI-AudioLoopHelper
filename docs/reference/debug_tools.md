...
| `trim_video_latent_to_audio_present` (F14) | `apply_trim_video_latent_to_audio.py` | Loop workflow's VAE decoder LATENT input not fed by `TrimVideoLatentToAudio`. Without the trim, fixed-stride iter math overshoots audio length by up to `window − stride` seconds; ffmpeg `-shortest` doesn't truncate `-c:v copy`, so the saved mp4 ends with audible silence. Migrated 2026-05-10 from image-space `TrimImageBatchToAudio` to latent-space — same correctness contract, lower decode VRAM/time. Postmortem: `internal/analysis/loop_audio_overshoot_analysis.md` (private clone only). |
