...
|---|---|---|
| `sage` / `sage_mode` / `sage_active` | `scripts/apply_sage_mode_auto.py` | Missing or non-`auto` AudioLoopHelperSageAttention (shipped default since 2026-05-15; runtime-equivalent to the node's `auto_mask_aware` schema default on this pipeline — see `docs/reference/sage_attention.md`) |
...
| `cond_frame_rate_24` (F16) | `apply_fps_24_default.py` | Asserts `LTXVConditioning.frame_rate == 25` (`LTX23_FPS` in `workflow_utils.py`; `apply_fps_24_default.py`'s `TARGET_FPS = LTX23_FPS`). The "24" in the check ID and script name is a legacy naming artifact from the pre-2026-05-16 revert — not a behavior gap; rename still TODO. Load-bearing for the model's temporal positional embedding (`comfy/ldm/lightricks/av_model.py:866` scales the time axis by `1/frame_rate`). Lightricks's shipped ComfyUI-LTXVideo example workflows set `LTXVConditioning.frame_rate=25` across T2V/I2V distilled + full; V2V uses 24 (preserves source-video fps). Canonical-evidence detail: `docs/reference/ltx23_model_reference.md` § "`frame_rate`: canonical inference value is 25". Audio-VAE internal `25 latents/sec` is independent of video fps. |
