...
| `cond_frame_rate_24` (F16) | `apply_fps_24_default.py` | `LTXVConditioning.frame_rate != 24`. Load-bearing for the model's temporal positional embedding (`comfy/ldm/lightricks/av_model.py:866` scales the time axis by `1/frame_rate`); LTX 2.3 was trained at 24fps. Pre-2026-05-15 default of 25 produced a ~4.2% pos-embed time-stretch. Sweep flips fps widgets across every fps-bearing node (see `LIST_WIDGET_NODES` in the apply script). A stale allowlist missing `LTXVAudioVideoMask` caused the 35-40s audio-video sync drift class of bug post-migration (KJNodes upstream node retained `video_fps=25`); `LTXHeadTrim` and `LatentSeamZoneMask` were added in the same pass. Regression guard: `tests/test_node_schemas.py::test_apply_fps_24_default_covers_all_fps_bearing_widgets`. Audio-VAE internal `25 latents/sec` is independent of video fps and stays. |
