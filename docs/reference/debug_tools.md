...
| `cond_frame_rate_24` (F16) | `apply_fps_24_default.py` | `LTXVConditioning.frame_rate != 24`. Load-bearing for the model's temporal positional embedding (`comfy/ldm/lightricks/av_model.py:866` scales the time axis by `1/frame_rate`); LTX 2.3 was trained at 24fps. Pre-2026-05-15 default of 25 produced a ~4.2% pos-embed time-stretch. Sweep also flips fps widgets on every other node that participates in the time axis (LTXFramePlanner, AudioLoopController/Planner, TimestampPromptScheduleBatchEncode, VHS_VideoCombine output mux, TrimVideo/ImageBatchToAudio, LatentTemporalMask, LoopConfigValidator, LTXVEmptyLatentAudio). Audio-VAE internal `25 latents/sec` is independent of video fps and stays. |
