...
| `trim_video_latent_to_audio_present` + `trim_image_batch_to_audio_present` (F14, layered) | `apply_trim_video_latent_to_audio.py` + `apply_trim_image_batch_to_audio.py` | Loop workflows must have BOTH trims wired. **Latent trim** (pre-VAE-decode) snaps UP to the smallest LTX-valid count where decoded pixels ≥ `int(audio*fps)` — saves decode VRAM/time on overshoot frames. **Image trim** (post-decode) clips the 0-7 pixel-frame residue from snap-UP to exact audio length. Without the layered pair, ffmpeg `-shortest` either clips audio (when video < audio) or leaves silence at end (when video > audio and `-c:v copy` defeats `-shortest`). Reverted to layered architecture on 2026-05-10 after Option A (latent-only) caused user-reported audio clipping. Postmortem: `internal/analysis/loop_audio_overshoot_analysis.md` (private clone only). |
| `pre_decode_cleanup_present` (WARN) | `apply_pre_decode_cleanup.py` | Full-song LOOP workflows must feed `TrimVideoLatentToAudio.latent` from `PreDecodeCleanup` (unload models + free pinned staging before the final decode's single-node RAM spike — otherwise the kernel can OOM-kill the process at the last step, after all sampling succeeded). Scope: active-TensorLoopOpen workflows only; single-pass/battery workflows deliberately exempt (cold model reload per prompt). Mechanism: [`benchmarking_memory_pressure.md`](benchmarking_memory_pressure.md). |
| `latent_checkpoint` (WARN) | `apply_latent_checkpoint.py` | Loop workflows must bank the assembled latent exactly once: `PreDecodeCleanup` widgets `[mode, checkpoint_keep, checkpoint_prefix]` with keep > 0 (core-SaveLatent-compatible file at `latents/checkpoints/<workflow>_NNNNN_.latent`, rotated keep-newest-N). WARNs both ways — keep=0 with no active `SaveLatent` (a decode-stage death loses the render's sampling) AND keep>0 alongside an active `SaveLatent` (double-saving; unbounded `.latent` accumulation). |
| `run_id_layout_present` (F15) | `apply_run_id_layout.py` | Loop workflow's `VHS_VideoCombine.filename_prefix` not fed by `RunIdPrefix.video_prefix`. WARN-level — without it, every render's mp4 + workflow-snapshot + audio-mux outputs spray flat with a global counter instead of clustering under `<output>/<workflow_name>/<timestamp>/`. Latent banking for the LoadLatent upscale/recovery path lives on `PreDecodeCleanup.checkpoint_keep` (see the `latent_checkpoint` row); the apply script skips its legacy SaveLatent step when checkpointing is configured. User guide: [`guides/upscale_guide.md`](../guides/upscale_guide.md). |
| `cond_frame_rate_24` (F16) | `apply_fps_24_default.py` | **PENDING UPDATE (2026-05-16) — currently asserts `frame_rate == 24` but the LTX 2.3 canonical inference value is 25.** Load-bearing for the model's temporal positional embedding (`comfy/ldm/lightricks/av_model.py:866` scales the time axis by `1/frame_rate`). Lightricks's shipped ComfyUI-LTXVideo example workflows set `LTXVConditioning.frame_rate=25` across T2V/I2V distilled + full; V2V uses 24 (preserves source-video fps). Canonical-evidence detail: `docs/reference/ltx23_model_reference.md` § "`frame_rate`: canonical inference value is 25". A 2026-05-15 sweep flipped widgets to 24 across all shipped workflows based on a misread of a library placeholder default; symptom was initial render motion looking fine, then loop iterations collapsing to near-still. Reverted to 25 on 2026-05-16; production render validation pending. The audit check, apply script name, regression guard, and `LIST_WIDGET_NODES` allowlist (covering `LTXVAudioVideoMask`, `LTXHeadTrim`, `LatentSeamZoneMask`, etc.) will be renamed/retargeted in a follow-up — out of scope for the 2026-05-16 docs sweep. Audio-VAE internal `25 latents/sec` is independent of video fps. |
| `cfg_guider_inputs_traced_to_source` (F17) | (no apply script — diagnostic invariant) | Loop-body `CFGGuider` (`#644` in canonical, plus any future variants inside the subgraph) has at least one of `.positive` / `.negative` / `.model` not traceable back to a real source. Walks backwards through subgraph links hopping through bypassed pass-through nodes + cropguides; conditioning chains terminate at a `CONDITIONING`-producing source type (e.g. `CLIPTextEncode`, `TimestampPromptScheduleBatchEncode`, `ConditioningSelectByIteration`, `LTXVCropGuides`) or at the subgraph distributor (`-10`); model chain continues on top-level past the invoker through model-passthrough nodes (`LoraLoaderModelOnly`, `LTXVReferenceAudio`, `LTXICLoRALoaderModelOnly`, `LoopIterationStamp`, `LTX2_NAG`) and `SetNode`/`GetNode` broadcasts to a `UNETLoader` (or equivalent). If `LTX2_NAG` is present and active in the workflow but the model walk doesn't pass through it, NAG isn't being applied to per-iter sampling — separate ERR. ERR-level. |
| `fps_coherence` (F18) | `apply_fps_24_default.py` (re-sweep) | Two or more fps/frame_rate widgets in the same workflow disagree. F16 enforces the absolute rule (asserted `== 24` until the 2026-05-16 revert; canonical inference value is 25, follow-up pending); F18 enforces relative coherence — every fps-bearing node listed in `apply_fps_24_default.LIST_WIDGET_NODES` plus `VHS_VideoCombine.widgets_values["frame_rate"]` must report the same int. Catches future drift even if the absolute target changes. ERR-level. |
| `keyframe_target_iters_set` (F19) | `apply_keyframe_iter_anchor.py` | Every `LTXIterKeyframeSchedule` row's `target_iters` is empty, so no row ever matches `current_iteration` and the selector returns its `fallback_latent` (init image) on every iteration — the wired keyframes silently never fire ("only one image in use"). WARN-level: the workflow still RUNS, it just ignores the keyframes. The apply script defaults `target_iters` to firing values (1,2,3); this guards a row being cleared back to empty. Design + footgun: [`../../example_workflows/working_docs/keyframe_iter_anchor_design.md`](../../example_workflows/working_docs/keyframe_iter_anchor_design.md). |
