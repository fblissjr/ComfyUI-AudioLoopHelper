...
- **`target_seconds` is per-iteration window duration, NOT total video length.** Total length is determined by the audio. Default 19.88 = 497 frames @ 25fps ≈ 9 iterations on a 3-min song.
...
- **`fps=25` is Lightricks's distilled default.** 24 (cinema) is valid but moves slightly off the model's training distribution.
