...
- `start_experiment.sh` (repo root) — companion wrapper to `scripts/startup/start.sh`. Sets `RUN_ID` + `AUDIOLOOPHELPER_SAGE_TRACE=auto` + `COMFYUI_EXEC_LOG=auto` env vars, cleans prior `trace.json` files, then exec's the underlying `start.sh`. Producer of the `data/runs/${RUN_ID}/${prompt_id}/sage.jsonl` files that `analyze_sage_traces.py` consumes. Has its own ~70-line header docstring covering env-var semantics + retention.
...
- `scripts/analyze_ffn_attn_trace.py [<jsonl>]` — aggregate per-stage FFN-vs-attention GPU-time breakdown from `ffn_attn_breakdown.jsonl` traces (produced by `tracers/ffn_attn.py` when `AUDIOLOOPHELPER_FFN_ATTN_TRACE` is set). Stage cutoff at T=16384.
- `scripts/analyze_torch_profile.py <torch_profile.N.json> [--modules ...]` — per-module-path aten-op breakdown from torch.profiler chrome traces (produced by `tracers/torch_profile.py`). Uses `record_function` annotations from `tracers/ffn_attn.py` for eager-mode module attribution (`with_modules=True` is TorchScript-only). **Do NOT delete raw chrome traces before running this** — op-class summaries from extraction are GLOBAL aggregates only, not per-module.
- `scripts/cleanup_traces.py --keep N [--pattern G] [--apply] [--no-extract]` — retention for `data/runs/` RUN_ID dirs. Default keeps last 20; dry-run unless `--apply`. `--apply` automatically runs `analyze_torch_profile.aggregate_by_module` against any raw chrome trace it finds and writes a `<stem>.modules_summary.json` sidecar BEFORE rm-tree, so per-module aten-op data survives retention. `--no-extract` is the escape hatch when the user has already extracted manually.
- `scripts/startup/start.sh` — canonical deploy template for ComfyUI's launcher (six modes: `default | safe | extreme | minimal | nodynvram | highvram`). `default` keeps dynamic VRAM ON (the no-dynvram flags were promoted into default 2026-06-04 and REVERTED the same day — the decode-stage OOM they were blamed for was decode buffer stacking, independent of paging, but dynamic VRAM still provides the offload headroom loop renders rely on; do-not-re-promote rationale inline at `NODYNVRAM_ARGS`). `nodynvram` (alias `bench`) is the kernel-OOM/bench config: perf flags + the no-dynvram kill switch + `--cache-none` + reserve 0. The launcher surfaces the exit status (137 = SIGKILL/OOM-killer) after the grep pipe. Full methodology at `docs/reference/benchmarking_memory_pressure.md`. **`nodynvram`/`safe`/`minimal` set `--cache-none` — benchmark-only; NEVER for a looping/full-song render (TensorLoop re-runs all upstream nodes per iter → N× slowdown). The script prints a `--cache-none active` guard. Use `default` for loop renders. Gotcha: `docs/reference/debug_tools.md`.**
...
| `promote_latent_for_upscale.py` | Find the most recent banked `.latent` for a workflow (PreDecodeCleanup checkpoints under `latents/checkpoints/`, plus the legacy per-render SaveLatent layout) and copy it to ComfyUI's input dir under a deterministic name. Reads `COMFYUI_OUTPUT_DIR` / `COMFYUI_INPUT_DIR` env or `--output-dir` / `--input-dir` flags · `docs/guides/upscale_guide.md` |
...
| `apply_pre_decode_cleanup.py` | Splice `PreDecodeCleanup` (unload models + free pinned staging) upstream of `TrimVideoLatentToAudio` across the full-song LOOP family — RAM/VRAM hygiene before the final decode (NOT the decode-OOM fix, which is temporal-chunked decode; see `benchmarking_memory_pressure.md`). Single-pass/short-clip workflows deliberately exempt (battery renders would cold-reload per prompt). Audit: `pre_decode_cleanup_present` (WARN) · `audit_workflows.py`, `docs/reference/benchmarking_memory_pressure.md` |
...
| `apply_run_id_layout.py` (F15) | Insert `RunIdPrefix` and wire it into `VHS_VideoCombine.filename_prefix` (+ any existing `SaveLatent`) so every render's artifacts cluster under `<output>/<workflow_name>/<timestamp>/`. Skips its legacy add-a-SaveLatent step when `PreDecodeCleanup.checkpoint_keep > 0` (banking moved to checkpoints) · `audit_workflows.py`, `docs/guides/upscale_guide.md` |
| `apply_latent_checkpoint.py` | Move decode-crash latent banking from the standalone always-on `SaveLatent` (unbounded per-render .latent accumulation) onto `PreDecodeCleanup`'s checkpoint widgets — `[mode, keep=2, latents/checkpoints/<stem>]`, rotated keep-newest-N; removes SaveLatent nodes. Audit: `latent_checkpoint` (WARN) · `audit_workflows.py`, `docs/guides/upscale_guide.md`, `docs/reference/benchmarking_memory_pressure.md` |
