...
- `scripts/analyze_ffn_attn_trace.py [<jsonl>]` — aggregate per-stage FFN-vs-attention GPU-time breakdown from `ffn_attn_breakdown.jsonl` traces (produced by `ffn_attn_tracer.py` when `AUDIOLOOPHELPER_FFN_ATTN_TRACE` is set). Stage cutoff at T=16384; verdict bucket maps to the v0.6 FFN-fusion decision-gate thresholds. Source data: PyTorch forward-hook timing on every `BasicAVTransformerBlock` sub-module.
