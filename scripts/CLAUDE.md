...
- `scripts/cleanup_traces.py --keep N [--pattern G] [--apply]` — retention for `data/runs/` RUN_ID dirs. Default keeps last 20; dry-run unless `--apply`. Extract per-module data via the analyzer BEFORE deleting chrome traces if it's still needed.
