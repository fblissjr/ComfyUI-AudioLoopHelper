...
| `apply_pre_decode_cleanup.py` | Splice `PreDecodeCleanup` (unload models + free pinned staging) upstream of `TrimVideoLatentToAudio` across the full-song LOOP family — the final decode's single-node RAM spike can kernel-OOM at the last step otherwise. Single-pass/short-clip workflows deliberately exempt (battery renders would cold-reload per prompt). Audit: `pre_decode_cleanup_present` (WARN) · `audit_workflows.py`, `docs/reference/benchmarking_memory_pressure.md` |
